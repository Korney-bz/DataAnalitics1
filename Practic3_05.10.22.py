...
class Warehouse:
    ...
    def act(self):
        while self.queue_out:
            ...
            truck.to_go(road=self.road_out)
...
class Truck(Vehicle):
    ...
    def ride(self):
        ...
        if self.distance_to_target > self.velocity:
            ...
            print('{} в дороге, осталось {}'.format(self.model, self.distance_to_target))
        else:
...
class AutoLoader(Vehicle):
    ...
    def __str__(self):
        res = super().__str__()
        return res + ' загружаем {}'.format(self.truck)
    ...
    def act(self):
        if super().act():
            if self.truck is None:
                ...
                if self.truck is None:
                    print('{} нет грузовиков для работы {}'.format(self.model, self.truck))
